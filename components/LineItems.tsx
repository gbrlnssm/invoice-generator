...
export default function LineItems({ data, onChange }: LineItemsProps) {
  ...
  const addItem = () => {
    onChange({
      ...
      lineItems: [
        ...data.lineItems,
        { id: crypto.randomUUID(), description: "", details: "", quantity: 1, rate: 0 },
      ],
    });
  };
      ...
      <div className="space-y-3">
        {data.lineItems.map((item) => (
          <div key={item.id} className="space-y-1.5">
            ...
            <div className="grid grid-cols-[1fr_70px_90px_90px_32px] gap-2">
              <textarea
                value={item.details}
                onChange={(e) => updateItem(item.id, "details", e.target.value)}
                placeholder="Description (optional)"
                rows={2}
                className="border border-gray-200 rounded-md px-3 py-1.5 text-[13px] text-[rgb(29,29,31)] placeholder:text-gray-400 focus:outline-none focus:border-gray-400 transition-colors resize-none"
              />
            </div>
          </div>
        ))}
